...
VALUES ('3.6.14','2.5.25',GETDATE());

/* TAREFA #23383 - FELIPE */

--Tabelas 
ALTER TABLE [RelBaseTrib]
ADD [RelBaseTribEstPacCod] SMALLINT    NULL

ALTER TABLE [PRF01]
ADD [PrfCustoPVMob] SMALLMONEY    NULL,
    [PrfCustoTrnGSurf] SMALLMONEY    NULL,
    [PrfCustoTrnMob] SMALLMONEY    NULL
	
	
--Procedure
Alter PROCEDURE InsereRelBaseTrib (@DataIni DATE, @DataFim DATE)
AS
BEGIN
	...
	(RelBaseTribDataRef, RelBaseTribEstCod, RelBaseTribSegmento,RelBaseTribEstPacCod, RelBaseTribValorTrn, RelBaseTribValorAdq, RelBaseTribValorParcPort, RelBaseTribValorPS, RelBaseTribAdqNom)
	SELECT
	Trn.MovTrnDta,
	Trn.EstCod, 
	Trn.EstSegmento,
	...
	(
		...
		SELECT A.EstCod, A.MovTrnDta, C.EstSegmento, C.EstPacCod, SUM(A.MovTrnVlr) 'ValorTrn'
		FROM MovTrn01 A
		INNER JOIN EST C
		ON A.EstCod = C.EstCod
		GROUP BY A.EstCod, A.MovTrnDta, C.EstSegmento, C.EstPacCod
	) Trn
	LEFT JOIN 
	(
