...
/****** Object:  View [dbo].[VwRelAudCessao]    Script Date: 29/06/2023 15:35:11 ******/
DROP VIEW [dbo].[VwRelAudCessao]
GO
...
GO
 ...
 CREATE view [dbo].[VwRelAudCessao] as
	...
	E.VlpVlrPag AS 'VwRelAudVlpVlrLiq',
	E.VlpVlrParBru AS 'VwRelAudVlpVlrBru'
	FROM CESSAO A 
	INNER JOIN EST B
...
/****** Object:  View [dbo].[VwRtSaldo]    Script Date: 29/06/2023 15:34:34 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[VwRtSaldo]
AS

SELECT
NEWID() 'VwRtSaldoId',
A.RtAnaliticoEstCod 'VwRtSaldoEstCod',
CONVERT(VARCHAR(10), A.RtAnaliticoEstCod) + ' - ' + D.EstNomFan 'VwRtSaldoEc',
A.RtAnaliticoDataTrn 'VwRtSaldoDataTrn',
B.MovTrnHra 'VwRtSaldoHoraTrn',
A.RtAnaliticoNsu 'VwRtSaldoNSU',
A.RtAnaliticoAutcod 'VwRtSaldoAutorizacao',
CASE
	WHEN A.RtAnaliticoProduto = 'D' THEN 'Debito'
	WHEN A.RtAnaliticoProduto = 'P' THEN 'Pix'
	WHEN A.RtAnaliticoProduto = 'C' AND B.MovTrnParQtd < 2 THEN 'Credito a vista'
	WHEN A.RtAnaliticoProduto = 'C' AND B.MovTrnParQtd >= 2 THEN 'Credito parcelado'
	WHEN A.RtAnaliticoProduto = 'S' THEN 'Prestação serviço'
	ELSE 'Outros'
END 'VwRtSaldoProduto',
CASE
	WHEN A.RtAnaliticoBandeira = 'V' THEN 'VISA'
	WHEN A.RtAnaliticoBandeira = 'M' THEN 'MASTERCARD'
	WHEN A.RtAnaliticoBandeira = 'E' THEN 'ELO'
	WHEN A.RtAnaliticoBandeira = 'A' THEN 'AMEX'
	WHEN A.RtAnaliticoBandeira = 'C' THEN 'CABAL'
	WHEN A.RtAnaliticoBandeira = 'H' THEN 'HIPERCARD'
	WHEN A.RtAnaliticoBandeira = 'P' THEN 'PIX'
END 'VwRtSaldoBandeira',
CASE 
	WHEN A.RtAnaliticoTrnCod IN ('CV', 'PS') AND A.RtAnaliticoProduto <> 'Cessao' THEN 'Venda'
	WHEN A.RtAnaliticoTrnCod = 'CC' AND A.RtAnaliticoProduto <> 'Cessao' THEN 'Cancelamento'
	WHEN A.RtAnaliticoTrnCod = 'AJ' AND A.RtAnaliticoProduto <> 'Cessao' THEN 'Ajuste'
	ELSE 'Cessao'
END 'VwRtSaldoTipo',
A.RtAnaliticoValorTrn 'VwRtSaldoValorBruto',
A.RtAnaliticoValorTrn - A.RtAnaliticoValorLiqEst 'VwRtSaldoTaxa',
A.RtAnaliticoValorLiqEst 'VwRtSaldoValorLiquido',
B.MovTrnParQtd 'VwRtSaldoParcela',
C.VanTrnPosIde 'VwRtSaldoTerminal',
A.RtAnaliticoValorPago + A.RtAnaliticoValorPagoBnf 'VwRtSaldoValorPago',
A.RtAnaliticoValorAberto 'VwRtSaldoValorAberto',
A.RtAnaliticoValorCancelado 'VwRtSaldoValorCancelado',
A.RtAnaliticoValorAnt 'VwRtSaldoValorAnt',
A.RtAnaliticoCessao 'VwRtSaldoValorCessao'
FROM RtAnalitico A
LEFT JOIN MovTrn01 B
ON A.RtAnaliticoNsu = B.MovTrnNsu AND A.RtAnaliticoAutcod = B.MovTrnAutCodStr AND A.RtAnaliticoTrnCod = B.MovTrnCod AND A.RtAnaliticoEstCod = B.EstCod
LEFT JOIN VAN04 C
ON B.MovTrnId = C.VanTrnMovId
LEFT JOIN EST D
ON A.RtAnaliticoEstCod = D.EstCod
...
insert into parsis (ParCod, ParDsc, ParTipPar, ParTamPar, ParCon) values
('DIR_EXCEL_TEMPLATE', 'CAMINHO TEMPLATE EXCEL AUDITORIA', 'CA', 500, '/usr/local/tomcat/webapps/prontoev15/static/Resources/Modelos/'),
('TELA_AUDITORIA_SALDO', 'TELA AUDITORIA LOJISTA', 'CA', 500, 'S')


USE [Pronto]
GO

/****** Object:  StoredProcedure [dbo].[RELTEMPAUDITORIA]    Script Date: 30/06/2023 10:52:46 ******/
...
RtAnaliticoValorAberto, RtAnaliticoValorCancelado, RtAnaliticoCessao, RtAnaliticoValorAnt, RtAnaliticoCessaoBNF,
RtAnaliticoValorPagoBNF, RtAnaliticoValorConc, RtAnaliticoValorEstorno, RtAnaliticoGuid, RtAnaliticoDataInc, RtAnaliticoUsr)
SELECT A.EstCod, A.MovTrnNsu, A.MovTrnAutCodStr, A.MovTrnDta, iif(MovTrnTipPrd = 'P', 'PIX', 'AJUSTE'), A.MovTrnBan, A.MovTrnCod,
