...
SELECT 
	...
	COALESCE(E.ValorVlrPag, 0),
CASE 
	WHEN COALESCE(A.ValorVlrPag, 0) = (COALESCE(B.ValorArqDet, 0) + COALESCE(E.ValorVlrPag, 0)) AND C.ValorReceberPrv = D.ValorReceberRel THEN 'OK'
...
(
	...
	FROM ARQDET A
	WHERE A.ArbDetDtaCre BETWEEN @DATAINI AND @DATAFIM
	AND A.ArbDetCodSit = 0
	GROUP BY A.ArbDetDtaCre
) E ON DT.DataTemp = E.DataCredito
