...
LEFT JOIN (
	SELECT COALESCE(SUM(MovTrnTxaEstVlrPrv), 0) 'ClienteValorPrevisto',
	MovTrnDta
	FROM MovTrn01
	WHERE MovTrnDta BETWEEN @DATAINI AND @DATAFIM
	AND MovTrnCod = 'CV'
...
FROM #_DataTemporaria DT
LEFT JOIN (
	SELECT CONVERT(DATE, A.TRN08PSDATATRANSACAO) 'DataRelMob', SUM(MovTrnVlr) 'ValorRelMob'
	FROM TRN08PS A
	LEFT JOIN TRN08 B
	ON A.TRN08PSTOKEN = B.TRNTOK AND B.TRN08VanTrnSeq > 0
	LEFT JOIN MovTrn01 C
	ON B.TRN08VanTrnSeq = C.MovTrnVanSeq
	WHERE CONVERT(DATE, A.TRN08PSDATATRANSACAO) BETWEEN @DATAINI AND @DATAFIM
	AND A.TRN08PSStatus = 1
	GROUP BY CONVERT(DATE, A.TRN08PSDATATRANSACAO)) A
ON DT.DataTemp = A.DataRelMob
LEFT JOIN (
	SELECT CONVERT(DATE, A.TRN08PSDATATRANSACAO) 'DataConfMob', SUM(MovTrnVlr) 'ValorConfMob'
	FROM TRN08PS A
	LEFT JOIN TRN08 B
	ON A.TRN08PSTOKEN = B.TRNTOK AND B.TRN08VanTrnSeq > 0
	LEFT JOIN MovTrn01 C
	...
	WHERE CONVERT(DATE, A.TRN08PSDATATRANSACAO) BETWEEN @DATAINI AND @DATAFIM
	AND A.TRN08PSStatus = 1
	GROUP BY CONVERT(DATE, A.TRN08PSDATATRANSACAO)) B
ON DT.DataTemp = B.DataConfMob
LEFT JOIN (
	SELECT CONVERT(DATE, A.TRN08PSDATATRANSACAO) 'DataComRelMob', SUM(MovTrnVlrLiqEst) 'ValorComRelMob'
	FROM TRN08PS A
	LEFT JOIN TRN08 B
	ON A.TRN08PSTOKEN = B.TRNTOK AND B.TRN08VanTrnSeq > 0
	LEFT JOIN MovTrn01 C
	...
	WHERE CONVERT(DATE, A.TRN08PSDATATRANSACAO) BETWEEN @DATAINI AND @DATAFIM
	AND A.TRN08PSStatus = 1
	GROUP BY CONVERT(DATE, A.TRN08PSDATATRANSACAO)) C
ON DT.DataTemp = C.DataComRelMob
LEFT JOIN (
	SELECT CONVERT(DATE, A.TRN08PSDATATRANSACAO) 'DataComConfMob', SUM(MovTrnVlrLiqEst) 'ValorComConfMob' 
	FROM TRN08PS A
	LEFT JOIN TRN08 B
	ON A.TRN08PSTOKEN = B.TRNTOK AND B.TRN08VanTrnSeq > 0
	LEFT JOIN MovTrn01 C
	...
	WHERE CONVERT(DATE, A.TRN08PSDATATRANSACAO) BETWEEN @DATAINI AND @DATAFIM
	AND A.TRN08PSStatus = 1
	GROUP BY CONVERT(DATE, A.TRN08PSDATATRANSACAO)) D
